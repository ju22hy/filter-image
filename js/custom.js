...
images.map(function (image) {
  // console.log(image.split('-')[0]); // ex) [shoe, 1.jpg] : split을 주어 '-'를 기준으로 shoe는 인덱스 0번, 1.jpg는 인덱스 1번이 된 상태로 0번을 읽음
  const imageElement = `
    <div class="filter-image" data-filter="${image.split('-')[0]}">
      <span><img src="images/${image}" alt="${image.split('-')[0]}" /></span>
    </div>
`;
...
function activateFilter() {
  ...
  Array.from(imageElements).filter((imageElement) => {
    ...
    }, 100); // 시간 지연 함수(promise) : 첫 번째 피라미터 = callback 함수, 두 번째 피라미터 = 시간(밀리초)
  });
}
